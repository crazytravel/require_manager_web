...
const UserPage: React.FC = () => {
    ...
    const { loading, fetchedData } = useFetch(`/api/v1/users/`, [changedId, activeStatus, grantId, selectVisible]);
        ...
        <div className={styles.container}>
            <Table
                columns={[
                    ...
                    {
                        title: 'Roles', key: 'roles', render: (text, record) =>
                            record.roles?.map((role: string) => <Tag color="blue">{role}</Tag>)
                    },
                    {
                        title: 'Active', key: 'active', render: (text, record, index) => (
                            <Switch checked={record.active}
                                onChange={() => handleActiveAction(record.id, record.active)} />
                        )
                    },
                    {
                        ...
                        render: (text, record) => {
                            return (
                                <div>
                                    <Button onClick={() => handleGrantAction(record.id)} type="link">Assign Roles</Button>
                ...
                loading={loading}
            />
            <RoleSelect userId={grantId} visible={selectVisible} onCancel={() => setSelectVisible(false)} onSave={grantSaveHandler} />
        </div>
    )
}
